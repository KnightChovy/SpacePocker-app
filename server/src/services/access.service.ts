...
export default class AccessService {
  ...
  async forgotPassword(data: { email: string }) {
    ...
    if (foundUser) {
      ...
      const otpExpiresIn =
        (process.env.PASSWORD_RESET_OTP_EXPIRES_IN || "10m") as SignOptions["expiresIn"];
  ...
  async verifyForgotPasswordOtp(data: { otpToken: string; otp: string }) {
    ...
    if (!payload?.userId || payload?.purpose !== "password-otp" || !payload?.otpHash) {
    ...
    const resetTokenExpiresIn =
      (process.env.PASSWORD_RESET_EXPIRES_IN || "15m") as SignOptions["expiresIn"];
  ...
  async resetPassword(data: {
    ...
    if (!token || !newPassword || !confirmNewPassword) {
      throw new BadRequestError("token, newPassword and confirmNewPassword are required");
    }
    ...
    const foundUser = await this.userRepo.findByIdWithPassword(String(payload.userId));
    if (!foundUser) {
