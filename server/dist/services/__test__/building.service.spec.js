...
describe('BuildingService', () => {
    ...
    beforeEach(() => {
        ...
        buildingService = new building_service_1.BuildingService(mockRepo);
        jest.clearAllMocks();
    });
    describe('createBuilding', () => {
        ...
        it('should throw error if buildingName missing', async () => {
            await expect(buildingService.createBuilding({ ...validData, buildingName: '' })).rejects.toThrow('Name and address are required');
        });
        it('should throw error if address missing', async () => {
            await expect(buildingService.createBuilding({ ...validData, address: '' })).rejects.toThrow('Name and address are required');
        });
        it('should throw error if campus missing', async () => {
            await expect(buildingService.createBuilding({ ...validData, campus: '' })).rejects.toThrow('Campus is required');
        });
        it('should throw error if managerId missing', async () => {
            await expect(buildingService.createBuilding({ ...validData, managerId: '' })).rejects.toThrow('Manager ID is required');
        });
    });
    describe('getBuildingById', () => {
        ...
        it('should throw error if id empty', async () => {
            await expect(buildingService.getBuildingById('')).rejects.toThrow('Building ID is required!');
        });
        it('should throw error if building not found', async () => {
            mockRepo.findById.mockResolvedValue(null);
            await expect(buildingService.getBuildingById('999')).rejects.toThrow(error_response_1.NotFoundError);
        });
    });
    describe('getAllBuildings', () => {
        it('should get all buildings with default pagination', async () => {
            ...
            expect(mockRepo.findAll).toHaveBeenCalledWith(undefined, undefined, 10, 0);
        });
        ...
        it('should sort by field', async () => {
            ...
            expect(mockRepo.findAll).toHaveBeenCalledWith(undefined, { buildingName: 'desc' }, 10, 0);
        });
    ...
    describe('updateBuilding', () => {
        ...
        it('should throw error if id empty', async () => {
            await expect(buildingService.updateBuilding('', { buildingName: 'Updated' })).rejects.toThrow('Building ID is required!');
        });
        it('should throw error if building not found', async () => {
            mockRepo.findById.mockResolvedValue(null);
            await expect(buildingService.updateBuilding('999', { buildingName: 'Updated' })).rejects.toThrow(error_response_1.NotFoundError);
        });
    });
    describe('deleteBuilding', () => {
        ...
        it('should throw error if id empty', async () => {
            await expect(buildingService.deleteBuilding('')).rejects.toThrow('Building ID is required!!');
        });
        it('should throw error if building not found', async () => {
            mockRepo.findById.mockResolvedValue(null);
            await expect(buildingService.deleteBuilding('999')).rejects.toThrow(error_response_1.NotFoundError);
        });
    });
});
