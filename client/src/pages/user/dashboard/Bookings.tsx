...
const Bookings = () => {
  const [activeTab, setActiveTab] = useState<'Active' | 'Past' | 'Cancelled'>('Past');
    ...
    <div className="p-4 md:p-8 scroll-smooth">
      <div className="max-w-5xl mx-auto flex flex-col gap-8">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-text-sub-light dark:text-text-sub-dark text-base">View and manage your current and past space reservations.</p>
        </div>
          ...
          <div className="border-b border-border-light dark:border-border-dark">
            <nav className="flex gap-8">
              {(['Active', 'Past', 'Cancelled'] as const).map((tab) => (
                ...
                >
                  {tab} {tab === 'Active' && (
                    <span className="ml-1 px-1.5 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-bold">1</span>
                  )}
                </button>
              ))}
            </nav>
