...
const PaginationButton: React.FC<{ icon?: string; label?: string; active?: boolean; disabled?: boolean }> = ({ icon, label, active, disabled }) => (
  <button
    disabled={disabled}
  ...
  >
    {icon ? <span>{icon === 'left' ? <ChevronLeft className='h-5 w-5' /> : <ChevronRight className='h-5 w-5' />}</span> : label}
  </button>
);
