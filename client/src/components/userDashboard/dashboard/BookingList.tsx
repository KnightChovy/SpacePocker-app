import { Calendar, ChevronRight, History, MapPin, MessageSquareText, Pencil, Timer } from 'lucide-react';
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    Upcoming Bookings
                    <span className="text-xs font-medium text-text-sub-light bg-border-light dark:bg-border-dark px-2 py-0.5 rounded-full">3</span>
                </h2>
            ...
            <div className="flex flex-col gap-3">
                {BOOKINGSUSER.map((booking, idx) => (
                    <div key={idx} className="group flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark hover:border-primary/50 dark:hover:border-primary/50 shadow-sm hover:shadow-md transition-all duration-300">
                        ...
                        <div className="flex-1 w-full text-center sm:text-left">
                            <div className="flex flex-col sm:flex-row sm:justify-between items-center sm:items-start mb-1">
                                ...
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${idx === 0 ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800' :
                                    idx === 1 ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800' :
                                        'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 border border-gray-200 dark:border-gray-700'
                                    }`}>
                                    {idx === 0 ? 'Confirmed' : idx === 1 ? 'Pending Approval' : 'Completed'}
                                </span>
                            </div>
                            ...
                            <div className="flex items-center justify-center sm:justify-start gap-4 text-xs font-medium text-text-sub-light dark:text-text-sub-dark">
                                ...
                                {idx < 2 ? (
                                    <span className="flex items-center gap-1 bg-background-light dark:bg-background-dark px-2 py-1 rounded-md">
                                        <Timer className='h-3 w-3' /> {booking.startTime} - {booking.endTime}
                                    </span>
                                ) : (
