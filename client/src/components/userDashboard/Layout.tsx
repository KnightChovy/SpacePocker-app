...
import { BellRing, Bookmark, CalendarDays, ChevronDown, CreditCard, LayoutDashboard, LogOut, Menu, Plus, Search, Settings } from 'lucide-react';
        ...
        <div className="flex flex-col h-full p-6 justify-between">
          <div className="flex flex-col gap-8">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <h1 className="text-lg font-bold tracking-tight">SPACEPOCKER</h1>
                <p className="text-text-sub-light dark:text-text-sub-dark text-xs font-medium">Rental Platform</p>
              </div>
            </div>
            ...
            <nav className="flex flex-col gap-2">
              {navItems.map((item) => (
                <NavLink
                  ...
                  `}
                >
                  <span className={` text-[20px] transition-transform ${location.pathname !== item.path ? 'group-hover:scale-110' : ''}`}>
          ...
          <div>
            <div className="bg-background-light dark:bg-background-dark rounded-xl p-4 mb-4 border border-border-light dark:border-border-dark">
              ...
              <p className="text-xs text-text-sub-light dark:text-text-sub-dark mb-3 leading-relaxed">Contact our support team for any booking issues.</p>
              <button className="text-xs font-bold text-primary hover:text-primary-dark transition-colors">Contact Support</button>
            </div>
          ...
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <button className="flex items-center gap-3 pl-1 pr-2 py-1 rounded-full hover:bg-background-light dark:hover:bg-surface-dark transition-colors border border-transparent hover:border-border-light dark:hover:border-border-dark">
              ...
              <div className="hidden md:flex flex-col items-start">
                ...
                <span className="text-[10px] text-text-sub-light dark:text-text-sub-dark">{USER_INFO.plan}</span>
              </div>
              <ChevronDown />
            </button>
          </div>
