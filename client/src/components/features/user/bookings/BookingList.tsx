...
const BookingList = ({ bookings, requests }: BookingListProps) => {
  ...
  const selectedExtras = useMemo(() => {
    ...
    if (fromLocal) {
      ...
      return {
        amenityIds: Array.isArray(fromLocal.amenityIds) ? fromLocal.amenityIds : [],
        services,
      };
    }
  ...
  return (
    <div className="flex flex-col gap-5">
      {bookings.map((booking, idx) => {
        ...
        const isFeedbackSubmitted = !!req?.roomId && submittedRoomIds.has(req.roomId);
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="rounded-xl bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark p-3">
                      <div className="flex items-center justify-between">
                        <span className="text-text-sub-light dark:text-text-sub-dark">Start</span>
                        <span className="font-medium text-text-main-light dark:text-text-main-dark">
                          {new Date(selectedRequest.startTime).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-text-sub-light dark:text-text-sub-dark">End</span>
                        <span className="font-medium text-text-main-light dark:text-text-main-dark">
                          {new Date(selectedRequest.endTime).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-text-sub-light dark:text-text-sub-dark">Status</span>
                        <span className="font-medium text-text-main-light dark:text-text-main-dark">
                    ...
                    <div>
                      <div className="text-xs font-semibold mb-1">Selected amenities</div>
                      <div className="rounded-xl bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark p-3">
                        {selectedExtras.amenityIds.length ? (
                          ...
                          <div className="text-sm text-text-sub-light dark:text-text-sub-dark">—</div>
                        )}
                      </div>
                    </div>

                    <div>
                      <div className="text-xs font-semibold mb-1">Selected services</div>
                      <div className="rounded-xl bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark p-3">
                        {(() => {
                          const entries = Object.entries(selectedExtras.services).filter(
                            ([, qty]) => (qty ?? 0) > 0
                          );
